...
import { NytlexImage, Link, router } from "nytlex/vue";
...
const navLinks = [
  ...
  { label: 'Owner', href: 'https://mfraz.ovh', external: true },
];
...
// Função para forçar a atualização do caminho
const updatePath = () => {
...
// Variáveis para guardar a referência original do history
...
onMounted(() => {
  if (typeof window !== 'undefined') {
    ...
    // Intercepta as mudanças de rota do SPA (quando o <Link> é clicado)
...
onUnmounted(() => {
  if (typeof window !== 'undefined') {
    ...
    // Restaura as funções originais do history
